...
export class NewThemeComponent {
  ...
  onSubmit(): void {
    if(this.themeForm.invalid) {
      this.themeForm.form.markAllAsTouched();
      return;
    }
    ...
    this.apiService.createTheme({
      themeName: this.themeName, postText: this.postText
    }).subscribe({
      next: (theme) => {
      ...
      error: (err) => {
        this.isLoading = false;
        this.errorMessage = err.error?.message || 'Failed to create new theme. Please try again.';
      }
    });
  }
