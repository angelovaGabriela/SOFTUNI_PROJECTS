...
public class BookServiceImpl implements BookService {
    ...
    @Override
    public List<String> findByAgeRestriction(String ageRestriction) {
        AgeRestriction restriction = AgeRestriction.valueOf(ageRestriction.toUpperCase()); //без значение какво е било въведено, всичко ще стане с главни букви, защото и в класа с енумерациите е така.

        return bookRepository.findByAgeRestriction(restriction).stream()
                .map(book -> book.getTitle())
                .collect(Collectors.toList());
      // пускам stream за да взема само заглавието на всяка книга, а не всичко, което се съдържа в нея
     //  това обаче преточва много информация и взима само нужното, тромаво е.
   }
    ...
    @Override
    public List<Book> findAllWithPriceBetween(float lowerBound, float upperBound) {
