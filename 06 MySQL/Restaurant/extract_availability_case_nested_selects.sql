...
SELECT (SELECT table_id FROM orders WHERE table_id = tables.id LIMIT 1) as table_id, capacity, (SELECT COUNT(client_id) FROM orders_clients WHERE order_id IN (SELECT id FROM orders WHERE table_id = tables.id)) as count_clients, 
CASE
...
GROUP BY table_id, capacity, count_clients, availability
